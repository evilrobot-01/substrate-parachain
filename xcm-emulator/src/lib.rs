...
// PDD: static/global memory local to a thread
thread_local! {
	// PDD: in-process message queues > HashMaps of double-ended queues, keyed by network name
	...
	// PDD: relay block and para ids, keyed by network name
...
// PDD: Facade for pushing messages into thread-local message queues
...
macro_rules! decl_test_relay_chains {
	(
		...
		$(
			...
			// PDD: impl test externalities
			$crate::__impl_test_ext_for_relay_chain!($name, $genesis, $on_init, $api_version);
		)+
	};
}

// PDD: implement test externalities for relay chain (access Substrate externalities such as storage)
#[macro_export]
macro_rules! __impl_test_ext_for_relay_chain {
	...
	(@impl $name:ident, $genesis:expr, $on_init:expr, $api_version:ident, $ext_name:ident) => {
		thread_local! {
			// PDD: initialise externalities using genesis
		...
		impl TestExt for $name {
			...
			fn execute_with<R>(execute: impl FnOnce() -> R) -> R {
				...
				// PDD: collect/deliver downward messages to destination(s)
				...
				$ext_name.with(|v| {
					v.borrow_mut().execute_with(|| {
						...
						for para_id in<$name as NetworkComponent>::Network::para_ids() {
							...
							// emulator directly sends them to dest (not relayed).
						}
					})
				});

				// PDD: process messages on destinations across network
...
// PDD: relay chain impls (helper functions)
#[macro_export]
...
// PDD: implement parachain XCMP message handlers
#[macro_export]
	...
	($name:ident) => {
		impl $crate::XcmpMessageHandler for $name {
			...
			>(
				...
				$name::execute_with(|| {
					// PDD: forward to configured handler (pallet)
...
// PDD: implement test externalities for parachain (access Substrate externalities such as storage)
#[macro_export]
	...
	(@impl $name:ident, $genesis:expr, $on_init:expr, $ext_name:ident) => {
		...
		impl TestExt for $name {
			...
			fn execute_with<R>(execute: impl FnOnce() -> R) -> R {
				...
				// PDD: increment relay chain block number
				...
				$ext_name.with(|v| {
					v.borrow_mut().execute_with(|| {
						...
						// PDD: set validation data on parachain via inherents
				...
				// PDD: collect/deliver resulting messages to destination(s)
				// send messages if needed
				$ext_name.with(|v| {
					v.borrow_mut().execute_with(|| {
...
// PDD: parachain impls (helper functions)
#[macro_export]
		...
		impl $parachain {
			...
			fn prepare_for_xcmp() {
				...
				<Self as TestExt>::ext_wrapper(|| {
					...
					// PDD: set initial validation data on parachain via inherents
					let _ = <Self as Parachain>::ParachainSystem::set_validation_data(
						<Self as Parachain>::RuntimeOrigin::none(),
...
macro_rules! decl_test_networks {
	(
		...
		$(
			...
			// PDD: impl Network trait
			impl $crate::Network for $name {
				type Bridge = $bridge;

				// PDD: init called on each Chain.execute_with(..)
				fn init() {
				...
				fn process_messages() {
					// PDD: loop until all messages processed
					...
					while let Some((to_para_id, messages))
						...
						$(
							...
							if $crate::PARA_IDS.with(|b| b.borrow_mut().get_mut(stringify!($name)).unwrap().contains(&to_para_id)) && para_id == to_para_id {
								...
								if msgs.len() != 0 {
									// PDD: process messages on parachain
				...
				fn process_horizontal_messages() {
					...
					// PDD: process horizontal messages to each destination parachain
					while let Some((to_para_id, messages))
				...
				fn process_bridged_messages() {
					...
					while let Some(msg) = $crate::BRIDGED_MESSAGES.with(|b| b.borrow_mut().get_mut(stringify!($name)).unwrap().pop_front()) {
						...
						match dispatch_result {
							...
							Ok(()) => {
								// PDD: notify delivery on source message handler
			...
			// PDD: impl relay chain for network
			$crate::__impl_relay!($name, $relay_chain);

			// PDD: impl parachain(s) for network
			$(
	...
	(
		...
		$(
			...
			impl $crate::Bridge for $name {
				...
				// PDD: initialise, ensuring source and target networks are initialised
				fn init() {
					use $crate::{NetworkComponent, Network};
