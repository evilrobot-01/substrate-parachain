...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		#[pallet::call_index(0)]
		#[pallet::weight({0})]
		pub fn send(origin: OriginFor<T>, para: ParaId, payload: Vec<u8>) -> DispatchResult {
			ensure_root(origin)?;
			...
			log::debug!(target: "ping", "sending ping to para: {:?}, seq: {seq}", &para);
			match send_xcm::<T::XcmSender>(
				...
				Ok((hash, cost)) => {
					Pings::<T>::insert(seq, frame_system::Pallet::<T>::block_number());
		...
		pub fn ping(origin: OriginFor<T>, seq: u32, payload: Vec<u8>) -> DispatchResult {
			...
			log::debug!(target: "ping", "ping received from para: {:?}, seq: {seq}", &para);
			Self::deposit_event(Event::Pinged(para, seq, payload.clone()));
			log::debug!(target: "ping", "sending pong to para: {:?}, seq: {seq}", &para);
			match send_xcm::<T::XcmSender>(
				(Parent, Junction::Parachain(para.into())).into(),
				Xcm(vec![Transact {
					origin_kind: OriginKind::Native,
		...
		pub fn pong(origin: OriginFor<T>, seq: u32, payload: Vec<u8>) -> DispatchResult {
			...
			log::debug!(target: "ping", "pong received from para: {:?}, seq: {seq}", &para);
